...
function render(){poster.dataset.format=state.format;poster.className=`poster layout-${state.layout}`;if($('#safeToggle').checked)poster.classList.add('show-safe');$('#headlineOut').innerHTML=highlight(state.headline,state.accent);$('#guestNameOut').textContent=state.guestName;$('#guestRoleOut').textContent=state.guestRole;$('#epOut').textContent=String(state.episode).padStart(2,'0');$('#topicsOut').textContent=state.topics;photo.src=state.photoData||'';placeholder.style.display=state.photoData?'none':'grid';photo.style.objectPosition=`${state.posX}% ${state.posY}%`;photo.style.transform=`scale(${state.zoom/100})`;photo.style.filter=`brightness(${state.brightness}%) contrast(${state.contrast}%) grayscale(${state.grayscale}%) sepia(${state.warmth}%)`;mark.style.opacity=state.markOpacity/100;mark.style.transform=`translate(${state.markX}%,${state.markY}%) scale(${state.markScale/100})`;if(state.markData){customMark.src=state.markData;customMark.hidden=false;fallbackMark.style.display='none'}else{customMark.hidden=true;fallbackMark.style.display='block'}updateRangeOutputs();fit();save()}
...
function fit(){const stage=$('.stage-wrap'), shell=$('.preview-shell');if(!stage||!poster)return;const pad=window.innerWidth<880?16:52;const availW=Math.max(260,stage.clientWidth-pad),availH=window.innerWidth<880?Math.min(window.innerHeight*.68,720):Math.max(340,window.innerHeight-pad);const s=Math.min(availW/poster.offsetWidth,availH/poster.offsetHeight,1);$('#scale').style.transform=`scale(${s})`;shell.style.width=(poster.offsetWidth*s)+'px';shell.style.height=(poster.offsetHeight*s)+'px'}
